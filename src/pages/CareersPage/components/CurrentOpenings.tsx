      ...
      {Object.keys(grouped).map((k, i) => (
        <div key={i} className="space-y-8">
          ...
          <div className="space-y-4">
            {grouped[k].map((op, j) => (
              <a
                className="block group max-w-[1200px] "
                target="_blank"
                href={op.link}
                key={j}
              >
                <div className=" duration-200 ease-in-out rounded-lg xl:rounded-xl border border-light-black lg:px-8 lg:py-6 px-4 py-3 flex items-center justify-between gap-4 lg:gap-8 flex-wrap">
                  <div className="flex max-lg:flex-col lg:items-center lg:gap-20 gap-1 ">
                    <p className="text-24">{op.title}</p>
                    <p className="text-20 font-light opacity-60">
                      {op.type}
                      <span className="lg:hidden"> - {op.location}</span>
                    </p>
                  </div>
